...
function Filter({ data, setNewData, setnumbersPosts, setCurrentPage }) {
  ...
  const inputHandler = (e) => {
    ...
    const reNumber = /^([0-9]*)$/;
    const reLetters = /^[a-zA-Zа-яА-Я]+$/iu;
    if (
      filterColumn != "Название"
  ...
  return (
    <section className="sorting">
      ...
      >
        ...
        <option className="visible" value="Определенное значение">
          Определенное значение
        </option>
        <option
          className={filterColumn == "Название" ? "visible" : "invisible"}
          value="Включает в себя"
        >
          Включает в себя
        </option>
        <option
          className={filterColumn == "Название" ? "invisible" : "visible"}
          value="Больше"
        >
          Больше
        </option>
      ...
      <div className="block">
        ...
        <input
          className="input"
          value={filterInput}
          disabled={!filterColumn || !filterOption}
        ...
        <button
          ...
          disabled={
            !filterColumn || !filterOption || !filterInput || !inputDirty
          }
        >
          Показать
        </button>
